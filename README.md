...
The original LSKNet introduces a novel mechanism to dynamically adjust large spatial receptive fields, which is particularly effective for modeling long-range contextual dependencies in remote sensing images where objects may vary significantly in scale and orientation.

This modified version integrates additional improvements to further enhance model performance on remote sensing datasets with high foreground-background imbalance and inter-class variation.

## Key Modifications

### 1. Softmax Focal Loss Integration
To address the class imbalance issues common in remote sensing detection (e.g., rare object categories and dominant background), we have introduced **SoftmaxFocalLoss** into the training pipeline. This loss function effectively alleviates both foreground-background imbalance and class-level imbalance, leading to more stable and accurate detection results.

### 2. Enhanced Backbone with SE Module
We have improved the `LSKblock` structure by adding a **Squeeze-and-Excitation (SE)** module within the backbone network. This enhancement allows the model to better capture channel-wise feature dependencies and adaptively recalibrate channel attention weights. The SE module complements the existing spatial attention mechanisms in LSKNet and improves overall feature representation.
...
We thank the original authors of LSKNet and the MMRotate team for providing a solid foundation for this work. Our enhancements aim to extend the applicability and effectiveness of LSKNet in complex remote sensing scenarios.
