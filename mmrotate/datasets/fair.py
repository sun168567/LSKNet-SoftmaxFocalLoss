...
class FairDataset(CustomDataset):
    ...
    # def merge_det(self, results, nproc=4):
    #     """Merging patch bboxes into full image, adapting to sliding window format.
    ...
    #         # 解析新的命名格式 id_x_y.png，提取出 x 和 y 的偏移量
    #         splitname = img_id.split('_')
    #         oriname = splitname[0]
    #         x, y = int(splitname[1]), int(splitname[2].split('.')[0])
    ...
    #             # 修正坐标：加上当前窗口的偏移量 (x, y)
    #             ori_bboxes[..., :2] = ori_bboxes[..., :2] + np.array([512 * x, 512 * y], dtype=np.float32)
                
    #             # 给每个检测结果加上类别标签 (假设这里类别标签是从0开始的)
    #             labels = np.zeros((bboxes.shape[0], 1)) + i
    ...
    def get_ann_info(self, idx):
        """Override to ensure bboxes and labels are numpy arrays with correct shape."""
        ann = self.data_infos[idx]['ann']

        bboxes = np.array(ann['bboxes'], dtype=np.float32).reshape(-1, 5)
        labels = np.array(ann['labels'], dtype=np.int64)
        bboxes_ignore = np.array(ann.get('bboxes_ignore', []), dtype=np.float32).reshape(-1, 5)
        labels_ignore = np.array(ann.get('labels_ignore', []), dtype=np.int64)
